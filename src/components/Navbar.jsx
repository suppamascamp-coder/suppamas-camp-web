    ...
    <header className={`fixed w-full z-50 transition-all duration-300 ease-in-out ${isScrolled ? 'bg-white shadow-md py-3' : 'bg-transparent py-5'}`}>
        ...
        <div className="flex justify-between items-center">
          ...
          <nav className="hidden md:flex space-x-8">
            {['กิจกรรม', 'สถานที่', 'โปรแกรม', 'ติดต่อเรา'].map((item) => (
              <Link key={item} href={`#${item}`} className={`font-medium transition-colors hover:text-orange-500 ${isScrolled ? 'text-gray-600' : 'text-gray-200'}`}>{item}</Link>
            ))}
          </nav>
          <div className="hidden md:block">
            <button className="bg-orange-500 hover:bg-orange-600 text-white px-6 py-2.5 rounded-full font-semibold transition-transform transform hover:scale-105 shadow-md flex items-center gap-2">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-white shadow-xl flex flex-col items-center py-6 space-y-4 border-t border-gray-100">
          {['กิจกรรม', 'สถานที่', 'โปรแกรม', 'ติดต่อเรา'].map((item) => (
            <Link key={item} href={`#${item}`} onClick={() => setIsMobileMenuOpen(false)} className="text-gray-800 font-medium w-full text-center py-2 hover:text-orange-500">{item}</Link>
          ))}
          <button className="bg-orange-500 text-white px-8 py-3 rounded-full font-semibold w-10/12 mt-4">จองค่ายทันที</button>
        </div>
      )}
    </header>
  );
