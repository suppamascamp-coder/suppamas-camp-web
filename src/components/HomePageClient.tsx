...
// 📌 นำเข้า Firebase เพื่อดึงข้อมูลเพิ่มเติมในฝั่ง Client
...
export default function HomePageClient({ initialHomepageData, initialGalleryImages }: HomePageClientProps) {
  ...
  // 1. ข้อมูลหน้าแรก (Hero & Stats) - ใช้ข้อมูลที่ส่งมาจาก Server Component เลย
  const [homepageData, setHomepageData] = useState<any>(initialHomepageData);
  const [currentHeroSlide, setCurrentHeroSlide] = useState(0);
  
  // 2. ข้อมูลแกลลอรี่
  const [galleryImages, setGalleryImages] = useState<any[]>(initialGalleryImages || []);
  ...
  // ดึงข้อมูลแกลลอรี่ทั้งหมดเมื่อกด "ดูรูปภาพทั้งหมด"
  useEffect(() => {
    if (!showAllGallery) {
    ...
    const fetchAllGallery = async () => {
      ...
      try {
        ...
        const qGallery = query(baseQuery, orderBy("createdAt", "desc"));
    ...
    fetchAllGallery();
  }, [showAllGallery, initialGalleryImages]);
  ...
  const displayCards = homepageData?.highlightCards || [];
  const displayFeatures = homepageData?.featureCards || [];
  ...
  const allStaff = homepageData?.staffList || [];
        ...
        <div className="bg-white rounded-[2rem] shadow-2xl p-8 grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 border-b-8 border-orange-500">
          {[
            ...
            <div key={i} className={`text-center ${i !== 0 ? 'border-l border-gray-100' : ''}`}>
              ...
              <div className="text-[10px] md:text-xs text-gray-400 font-bold uppercase tracking-tighter mt-1 leading-none">{item.label || 'สถิติ'}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="activities" className="py-24 scroll-mt-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6 text-center md:text-left">
             <div>
                <h2 className="text-3xl md:text-5xl font-black text-green-950 mb-4 uppercase">{displayTexts.activityTitle || 'กิจกรรมไฮไลท์'}</h2>
      ...
      <section id="facilities" className="py-24 bg-green-950 text-white scroll-mt-24 overflow-hidden relative">
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10">
          <div className="text-center mb-20">
             <h2 className="text-3xl md:text-5xl font-black mb-6 uppercase tracking-tight italic">{displayTexts.facilityTitle || 'สิ่งอำนวยความสะดวก'}</h2>
